...
- Product Owner 于 2026-08-03 授予完整 V1 implementation authorization；I0-I5已经完成并分别通过
  ...
  [I4资格](conformance/i4-selection-mutation-resource-qualification.md)与
  [I5资格](conformance/i5-group-relation-qualification.md)；
- 当前没有active slice，I6-I8为`NOT_STARTED`，下一项从I6开始；
- G1、G4-G6为`PASS`；G2为I5范围、G3为I4 accounting范围，整体仍`IN_PROGRESS`；G10为I0范围
  `PASS`但整体仍`IN_PROGRESS`；G7-G9为`NOT_RUN`。
...
当前active checkout已有I0-I5 production source、Maven reactor、恰好两个production artifact、
完整I2 generated schema/type/Key/Index API breadth、paged primitive/reference PLAIN storage、Value
flattening、multiple Index、direct selection与point add/update/remove，以及sequential query、typed IR、
reference interpreter、optimized sequential execution、Selection mutation、Group accounting、GroupBy与
binary Equality/Cross Join的源码、测试和资格脚本；没有I6+ parallel、compression、benchmark、Example、
CI/release workflow、remote package或committed build artifact。I5完成不等于完整implementation、performance、
...
| Engineering | `project/engineering/`；I0-I5 completed，当前无active slice，下一项I6 |
| Conformance | `project/conformance/`；G1/G4-G6 PASS，G2/G3/G10 scoped PASS且整体IN_PROGRESS |
                ...
                -> I3 direct query/IR/reference interpreter (COMPLETED)
                    -> I4 selection mutation/resource/failure (COMPLETED)
                        -> I5 GroupBy/relation (COMPLETED)
                            -> I6 bounded parallel execution (NEXT)
...
I0-I5 exit evidence已经闭合；下一项只允许从I6开始。当前授权同时允许每个slice证据闭合后的
...
[I5 Qualification](conformance/i5-group-relation-qualification.md)。I6-I8的parallel、performance、
package和release Gate只能在相应surface出现后执行，不得用I0-I5 scoped
evidence替代。

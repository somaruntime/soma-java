...
- Reusable DataFlow 必须把 immutable Definition/Template 与 current one-shot Invocation 分开，并披露 Shape、Result/Effect、budget、parallel fallback 和 safe-point 边界；
...
当前可执行参考应用从 [`soma-examples`](../modules/soma-examples/README.md) 进入。它们的领域目标、算法、跨表提交顺序和失败处置由 application 自己拥有。

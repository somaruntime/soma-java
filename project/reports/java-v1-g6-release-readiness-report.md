...
输入事实源：release governance、private SCM、CI/workflow、binary/source
package与security scripts、current support matrix、retained qualification artifact
与AI consumer Skill
...
frontier scaling的声明边界；successor还包含compiler、DataFlow、项目组织与
workflow修复。新的clean immutable SHA必须重放适用Full、G5、binary/source
package、security、support matrix和manual qualification，条件式Owner sign-off
才会生效。
...
但不构成successor evidence。新增curated source archive也必须由successor自己的
retained bundle证明，Report不复制一份会漂移的动态run清单。
...
| package / security / provenance | same-SHA evidence-resolved | 最终SHA必须形成17件release-shaped artifact双构建，以及allowlist受控且从解包内容重建通过的curated source archive；classfile 52、License/NOTICE、SBOM、OSV、license与runtime dependency evidence必须一致 |
...
| Profile / claim | 状态 | 边界 |
|---|---|---|
| private GitHub source | conditional sign-off | 同SHA Full、G5、binary/source package、security/provenance、matrix、private CI、manual qualification与下载bundle checksum全部成功即passed，否则blocked |
...
Private-source profile仍验证binary/source package与security mechanics，因为它们
证明source能生成一致、可审计的release-shaped artifact、精简源码交付物及
dependency/provenance边界；这不等于选择Maven distribution，也不要求signing或
publishing。
...
3. 运行Full、两次隔离binary package build、curated source archive build与
   security scan；
4. 校验binary/source package与security summary都是同一SHA、`dirty=false`和
   同一version，并验证source archive内容边界；
...
binary/source package provenance、security summary与`evidence-checksums.sha256`
共同解析结果；任一缺失、commit/version/dirty不一致或Gate失败都保持`blocked`。
...
- 完整private repository保留`project/`事实与历史；curated source archive显式
  排除内部治理、repository tests、CI与raw evidence，并从archive本身完成构建验证；
- public Java type、module与runtime dependency增量均为零；package-private
  ...
  arithmetic Owner承载；未裁决产品语义为零，项目组织Temporary须在最终candidate
  admission前退役；

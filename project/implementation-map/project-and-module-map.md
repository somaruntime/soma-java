...
非事实范围：规范性模块语义、未来模块规划和某次 Gate 结果
...
根 [`pom.xml`](../../pom.xml) 当前版本为 `1.0.0`，Java source/target 1.8，并按以下顺序声明模块：
...
| `soma-examples` | [`pom.xml`](../../soma-examples/pom.xml)、[应用入口](../modules/soma-examples/README.md) | 三个独立 Java 8 reference consumer 的 aggregator；不产出共享领域 JAR |
...
- curated source package：
  [`scripts/source-package-smoke.sh`](../../scripts/source-package-smoke.sh)；

...
| 项目事实、角色投影与源码交付 | cutover已实现；final evidence-resolved | Blueprint/Design/Map/Conformance/Process/Report与模块事实集中在`project/`，用户投影集中在`docs/`；curated source archive采用allowlist并从解包内容重建。DOCX内嵌路径、Temporary退役与最终同SHA Gate仍必须闭合 |
...
| G6 selected private-source | conditional Owner sign-off | 当前successor已闭合产品语义与实现；同SHA Ubuntu Full、binary/source package、reproducibility、security/provenance、sealed bundle、support matrix、manual qualification与下载checksum全部成功才为passed，否则blocked |
...
项目组织cutover只改变事实位置、角色投影、checker和delivery allowlist，不改变
上述产品语义、代码实现或性能baseline。最终candidate admission仍要求DOCX内嵌
旧路径归零、Temporary退役、curated source archive从自身重建成功，并由同SHA
qualification artifact解析。
...
- [项目组织治理](../reports/project-organization-governance-report.md)

...
类型：Process / Maintainer Guide

状态：正式

Owner：SOMA Java 开发与维护过程
...
适用版本：`1.0.0`；当前核对基线为包含本文件的private-source sign-off
commit，精确commit由Git与同SHA qualification artifact记录
...
先从 [Blueprint](../blueprint/README.md) 确认目标，从 [Design](../design/README.md) 找规范性 Owner，再用 [Implementation Map](../implementation-map/README.md) 进入代码。发现偏差时记录 [Conformance](../conformance/README.md)，不要让当前代码反向成为设计理由。
...
./scripts/source-package-smoke.sh
```

完整验证使用Amazon Corretto 8.502.07.1 full JDK 8；不要求在Zulu或其他JDK
distribution上重放。窄检查适合快速反馈，跨模块/public/generated/protocol
收口运行`./scripts/check.sh`。

...
reject_match() {
    ...
    set -e
    case "$status" in
        0) echo "query-planning: forbidden surface detected" >&2; exit 1 ;;
        1) return 0 ;;
        *) exit "$status" ;;
    esac
}
...
qualification_scope=${SOMA_QUERY_QUALIFICATION_SCOPE:-query}
case "$qualification_scope" in
    query|mutation) ;;
    *) echo "invalid query qualification scope: $qualification_scope" >&2; exit 1 ;;
esac

if [ -z "${JAVA_HOME:-}" ]; then
    echo "$qualification_scope-qualification: JAVA_HOME must select the qualified Java 8 JDK" >&2
    exit 1
fi
...
for tool in "$java_cmd" "$javac_cmd" "$javap_cmd" "$jar_cmd"; do
    if [ ! -x "$tool" ]; then
        echo "$qualification_scope-qualification: missing JDK tool: $tool" >&2
        exit 1
    fi
done
if ! "$java_cmd" -version 2>&1 | grep -q 'version "1\.8\.' \
        || ! "$javac_cmd" -version 2>&1 | grep -q '^javac 1\.8\.'; then
    echo "$qualification_scope-qualification: JAVA_HOME is not a Java 8 JDK" >&2
    exit 1
fi
...
if [ -z "$runtime_jar" ] || [ -z "$processor_jar" ]; then
    echo "$qualification_scope-qualification: production artifacts not found" >&2
    exit 1
fi

work_root=$(mktemp -d "${TMPDIR:-/tmp}/soma-$qualification_scope-qualification.XXXXXX")
cleanup() {
    case "$work_root" in
        */soma-query-qualification.*|*/soma-mutation-qualification.*) rm -rf -- "$work_root" ;;
        *) echo "$qualification_scope-qualification: refusing unsafe cleanup target" >&2 ;;
    esac
}
...
find "$repo_root/tests/query-planning/src/main/java/example/i3/schema" \
...
set -- \
    "$repo_root/tests/query-planning/src/main/java/example/i3/I3ConsumerMain.java" \
    "$repo_root/tests/query-planning/src/main/java/example/i3/I3PlanGoldenMain.java"
if [ "$qualification_scope" = mutation ]; then
    set -- "$@" \
        "$repo_root/tests/mutation-resource/src/main/java/example/i3/I4ConsumerMain.java"
fi
"$javac_cmd" \
    -source 8 \
    -target 8 \
...
diff -u "$repo_root/tests/query-planning/golden/generated-source.sha256" \
    "$generated_hashes"
...
diff -u "$repo_root/tests/query-planning/golden/generated-public.classes" \
    "$generated_public_classes"
...
if grep -q 'io.github.somaruntime.soma.internal' "$generated_public_javap"; then
    echo "$qualification_scope-qualification: generated public signature leaks internal type" >&2
    exit 1
fi
...
diff -u "$repo_root/tests/query-planning/golden/generated-public.javap.sha256" \
    "$generated_public_digest_file"
...
diff -u \
    "$repo_root/tests/query-planning/golden/generated-public.javap.verbose.sha256" \
    "$generated_verbose_digest_file"
...
diff -u "$repo_root/tests/query-planning/golden/optimizer-plans.txt" \
...
find "$repo_root/tests/schema-storage/src/main/java/example/i2/schema" \
        -name '*.java' -type f -print \
    | LC_ALL=C sort > "$i2_schema_sources"
printf '%s\n' \
    "$repo_root/tests/schema-storage/src/main/java/example/i2/Status.java" \
    "$repo_root/tests/schema-storage/src/main/java/example/i2/Payload.java" \
    >> "$i2_schema_sources"
...
"$javac_cmd" \
    ...
    -d "$i2_classes" \
    "$repo_root/tests/schema-storage/src/main/java/example/i2/I2ConsumerMain.java" \
    "$repo_root/tests/schema-storage/src/main/java/example/i2/I2ScaleMain.java"
...
expect_compile_failure() {
    ...
    if "$javac_cmd" \
        ...
        echo "$qualification_scope-qualification: expected compile failure: $source_file" >&2
        exit 1
    fi
    if ! grep -Eq "$expected_pattern" "$negative_output"; then
        echo "$qualification_scope-qualification: expected diagnostic pattern missing: $expected_pattern" >&2
        cat "$negative_output" >&2
        exit 1
    fi
}

negative_root="$repo_root/tests/query-planning/src/negative/java/example/i3"
...
if [ "$qualification_scope" = query ]; then
...
find "$repo_root/tests/regeneration/schema-storage/snapshots/initial" \
        -name '*.java' -type f -print \
    | LC_ALL=C sort > "$work_root/regen-initial.txt"
find "$repo_root/tests/regeneration/schema-storage/snapshots/renamed" \
        -name '*.java' -type f -print \
...
diff -u "$repo_root/tests/query-planning/golden/runtime-public.classes" \
    "$runtime_public_classes"
...
if grep -q 'io.github.somaruntime.soma.internal' "$runtime_public_javap"; then
    echo "$qualification_scope-qualification: runtime public signature leaks internal type" >&2
    exit 1
fi
...
diff -u "$repo_root/tests/query-planning/golden/runtime-public.javap.sha256" \
    "$runtime_public_digest_file"
...
diff -u \
    "$repo_root/tests/query-planning/golden/runtime-public.javap.verbose.sha256" \
    "$runtime_verbose_digest_file"
...
if grep -Eq 'java/lang/reflect|java/lang/Class\.forName' \
    ...
    echo "$qualification_scope-qualification: processor depends on reflection" >&2
    exit 1
fi

...
reject_match() {
    ...
    set -e
    case "$status" in
        0) echo "group-relation: forbidden surface detected" >&2; exit 1 ;;
        1) return 0 ;;
        *) exit "$status" ;;
    esac
}
...
if [ -z "${JAVA_HOME:-}" ]; then
    echo "group-relation: JAVA_HOME must select the qualified Java 8 JDK" >&2
    exit 1
fi
...
if [ -z "$runtime_jar" ] || [ -z "$processor_jar" ]; then
    echo "group-relation: production artifacts not found" >&2
    exit 1
fi

work_root=$(mktemp -d "${TMPDIR:-/tmp}/soma-group-relation.XXXXXX")
cleanup() {
    case "$work_root" in
        */soma-group-relation.*) rm -rf -- "$work_root" ;;
        *) echo "group-relation: refusing unsafe cleanup target" >&2 ;;
    esac
}
...
expect_compile_failure() {
    ...
    if LC_ALL=C "$javac_cmd" \
        ...
        echo "group-relation: expected compile failure: $source_file" >&2
        exit 1
    fi
    if ! grep -Eq "$expected_pattern" "$negative_output"; then
        echo "group-relation: expected diagnostic missing: $expected_pattern" >&2
        cat "$negative_output" >&2
        exit 1
    fi
}

negative_root="$repo_root/tests/group-relation/src/negative/java/example/i5"
...
if grep -q 'io.github.somaruntime.soma.internal' \
    ...
    echo "group-relation: generated public signature leaks internal type" >&2
    exit 1
fi

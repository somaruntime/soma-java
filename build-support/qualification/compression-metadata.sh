...
if [ -z "${JAVA_HOME:-}" ]; then
    echo "compression-metadata: JAVA_HOME must select the qualified Java 8 JDK" >&2
    exit 1
fi
...
for tool in "$java_cmd" "$javac_cmd" "$javap_cmd" "$jar_cmd"; do
    test -x "$tool" || {
        echo "compression-metadata: missing JDK tool: $tool" >&2
        exit 1
    }
done
...
if [ "${SOMA_COMPRESSION_REUSE_BUILD:-0}" != 1 ]; then
    mvn clean package
fi
...
test -n "$runtime_jar" && test -n "$processor_jar" || {
    echo "compression-metadata: production artifacts not found" >&2
    exit 1
}

work_root=$(mktemp -d "${TMPDIR:-/tmp}/soma-compression-metadata.XXXXXX")
cleanup() {
    case "$work_root" in
        */soma-compression-metadata.*) rm -rf -- "$work_root" ;;
        *) echo "compression-metadata: refusing unsafe cleanup target" >&2 ;;
    esac
}
...
"$javac_cmd" -source 8 -target 8 -encoding UTF-8 -proc:none \
    ...
    "$repo_root/tests/compression-metadata/src/main/java/example/i5/I7ConsumerMain.java"
...
if grep -q 'io.github.somaruntime.soma.internal' "$generated_public"; then
    echo "compression-metadata: generated public signature leaks internal type" >&2
    exit 1
fi
...
if grep -q '^org/junit/' "$work_root/runtime-jar.txt" \
    ...
    echo "compression-metadata: JUnit leaked into a production artifact" >&2
    exit 1
fi
...
git diff --check
echo "compression-metadata: PASS"

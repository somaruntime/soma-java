...
final class CandidateParallelKernels {
    ...
    private CandidateParallelKernels() {
    }
    ...
    ExecutionOutcome<LongScalarResult> count(
        ...
        List<Integer> counts = BoundedMorselScheduler.run(
                frame,
                plan,
                new MorselWork<Integer>() {
                    @Override
    ...
    ExecutionOutcome<R> longReduction(
        ...
        final DataFlowBinding binding = frame.binding(program.source());
        final int cardinality = parallelCardinality(program, binding);
        final MorselPlan plan = BoundedMorselScheduler.plan(
                frame,
                cardinality,
                program.supportsContiguousParallel(),
                24L,
        ...
        List<LongPartial> partials = BoundedMorselScheduler.run(
                frame,
                plan,
                new MorselWork<LongPartial>() {
                    @Override

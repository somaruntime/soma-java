...
当前 repository baseline：`10a6107`
当前 checkpoint：Phase 1C dense performance shape / differential evidence
...
| Phase 1C dense shape/differential evidence | in-progress | primitive differential oracle、cursor reuse/fusion、bytecode/allocation-shape evidence；完成后才关闭 Phase 1 checkpoint |
...
当前 vertical slice：Phase 1C dense performance shape / differential evidence。
...
Slice exit：为 dense primitive/presence kernel 建立 deterministic differential oracle、cursor/pipeline fusion、generated bytecode 和 allocation-shape evidence；覆盖 remove/update/Column Pipeline/View 的 no per-row object/boxing contract，并把 Phase 1 的最终 architecture subset 与后续 keyed/sidecar/child work的 additive boundary审计收口。
...
计划 evidence：primitive/presence differential oracle、generated source/javap and bytecode inspection、allocation-shape harness、external Maven consumer、`./scripts/check.sh`。

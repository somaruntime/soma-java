...
完整 V1 implementation authorization。I0-I4已完成并分别通过qualification与审查闭环：
...
[I4资格](project/conformance/i4-selection-mutation-resource-qualification.md)：G1、G4、G5为`PASS`，
G2为I4范围、G3为I4 accounting范围、G10为I0范围`PASS`但整体仍`IN_PROGRESS`；I5-I8为
`NOT_STARTED`，当前没有active implementation slice，下一项只允许从I5开始。

Active checkout已包含I4 Selection/IndexSelection mutation、atomic StateRoot/Group accounting，以及I3
generated sequential query API、typed IR、reference interpreter、optimized execution和I2 schema/type/
Key/Index、paged PLAIN storage；仍不包含I5+ Group/Join、parallel/compression、benchmark、Example、
CI/release workflow、package或committed build artifact。I4 completion不等于完整implementation、performance、
...
- 当前 implementation authorization 覆盖 I0-I8 的自主实现；一次只推进一个 active slice；
  I0-I4已关闭，当前没有active slice，下一项只允许从I5开始；
...
- I0-I4 exit已通过；I5仍需按本计划关闭GroupBy与relation Gate，I3/I4的单Table证据不替代I5。
...
当前是post-I4 / pre-I5阶段。文档与repository-surface变更至少执行：

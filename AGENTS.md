...
完整 V1 implementation authorization。I0-I5已完成并分别通过qualification与审查闭环：
...
[I5资格](project/conformance/i5-group-relation-qualification.md)：G1、G4-G6为`PASS`，
G2为I5范围、G3为I4 accounting范围、G10为I0范围`PASS`但整体仍`IN_PROGRESS`；I6-I8为
`NOT_STARTED`，当前没有active implementation slice，下一项只允许从I6开始。

Active checkout已包含I5 GroupBy、binary Equality/Cross Join、typed result与relation reference/optimized
execution，以及I4 Selection/IndexSelection mutation、atomic StateRoot/Group accounting、I3 generated
sequential query API、typed IR和I2 schema/type/Key/Index、paged PLAIN storage；仍不包含I6+ parallel/
compression、benchmark、Example、CI/release workflow、package或committed build artifact。I5 completion不等于完整implementation、performance、
...
- 当前 implementation authorization 覆盖 I0-I8 的自主实现；一次只推进一个 active slice；
  I0-I5已关闭，当前没有active slice，下一项只允许从I6开始；
...
- I0-I5 exit已通过；I6仍需按本计划关闭bounded parallel Gate，I5的sequential relation证据不替代I6。
...
当前是post-I5 / pre-I6阶段。文档与repository-surface变更至少执行：

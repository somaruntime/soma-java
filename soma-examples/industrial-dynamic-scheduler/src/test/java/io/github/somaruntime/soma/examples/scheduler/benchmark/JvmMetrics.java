...
final class JvmMetrics {
  ...
  static long currentThreadAllocatedBytes() {
    try {
      ...
      long bytes = ((Long) THREAD_ALLOCATED.invoke(THREAD_BEAN,
      ...
      throw new IllegalStateException("cannot access allocation counter",
          failure);
    } catch (InvocationTargetException failure) {
      throw new IllegalStateException("allocation counter failed",
          failure.getCause());
    }
  }
  ...
  static GcSnapshot gcSnapshot() {
    ...
    return new GcSnapshot(youngCount, youngMillis, fullCount, fullMillis);
  }
  ...
  static final class GcSnapshot {
    ...
    GcSnapshot(long youngCount, long youngMillis,
               long fullCount, long fullMillis) {

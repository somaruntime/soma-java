...
/** Problem generation 配置的 strict file/classpath loader。 */
public final class ProblemConfigLoader {
  private static final Set<String> KEYS = Collections.unmodifiableSet(
      new HashSet<String>(Arrays.asList(
          "config.version", "generator.version", "seed", "jobs",
          "operations.per.job", "machines",
          "candidate.machines.per.operation", "setup.families",
          "secondary.resources", "secondary.resource.max.capacity",
          "secondary.resource.max.units", "processing.min.minutes",
          "processing.max.minutes", "setup.max.minutes",
          "transport.max.minutes", "release.spread.minutes",
          "material.delay.max.minutes", "maintenance.period.minutes",
          "maintenance.duration.minutes", "machine.delay.events",
          "machine.delay.max.minutes", "due.slack.minutes",
          "priority.levels")));

  private ProblemConfigLoader() {
  }

  public static ProblemGenerationConfig load(
      String selector, String... overrides) throws IOException {
    ...
    return new ProblemGenerationConfig(effective);
  }
  ...
  private static InputStream open(String selector) throws IOException {
    ...
    String resource = selector.endsWith(".properties")
        ? (selector.startsWith("config/") ? selector
            : "config/" + selector)
        : "config/" + selector + ".properties";
    InputStream input = ProblemConfigLoader.class.getClassLoader()

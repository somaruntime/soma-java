...
/** RTD benchmark artifact 的环境与 provenance 投影。 */
final class BenchmarkEnvironment {
  ...
  BenchmarkEnvironment(int minimumForks) {
    if (fork > configuredForks || configuredForks < minimumForks) {
      throw new IllegalArgumentException(
          "benchmark fork contract mismatch");
    }
  }
  ...
  private static void string(
      StringBuilder out, String name, String value) {
    quote(out, name);
    out.append(':');
    quote(out, value);
    out.append(',');
  ...
  private static void number(
      StringBuilder out, String name, int value) {
    quote(out, name);

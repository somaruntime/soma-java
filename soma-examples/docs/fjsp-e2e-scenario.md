...
5. 对 `MachineCandidate.scanByMachine(machineId)` 的候选更新 setup、effective ready time、FCFS value 和 SPT value；
...
9. 使用 `MachineCandidate.scanByOperation(operationKey).remove()` 删除该 operation 的全部候选；
...
`MachineCandidate` 是 keyed runtime frontier。候选存在表示该 `(MachineId, OperationKey)` 组合仍处于可选 frontier；operation 被选中后，solver loop 应通过 `scanByOperation(operationKey).remove()` 删除所有相关候选，而不是保留长期 `active` 标志。`indicatorReady` 只是当前 machine snapshot / 当前 dispatch 轮次下的 indicator 计算状态，不能作为长期业务状态或候选有效性事实。Dispatch rule 属于 solver 策略，示例不在 `MachineCandidate` schema 上声明 `byMachineDispatchRule` 这类 order。

Machine heap只保存`MachineId`到application slot的映射和派生availability，不保存SOMA物理Index。release成功路径激活相关machine；selected machine在table mutation后更新heap派生值；其他machine因frontier cleanup变空时允许保留stale entry，并在其到达heap root时按当前`scanByMachine(...).count()`惰性淘汰。任一跨table/queue步骤失败仍服从disposable instance policy，不把heap包装成SOMA transaction或`@SomaOrder`。
...
SOMA runtime 只提供 `find(...)` / `containsKey(...)` / empty Optional或Candidate result / typed missing error 等基础语义。当前 loader 还在发布 instance 前验证 job sequence 连续、引用完整、candidate machine 唯一、required setup transition matrix 完整、时间非负和 worst-case checked arithmetic；候选不可行、输入不完整、默认 setup 等业务解释属于 loader 或 solver core。
...
- `addBatch(batch)`；
- keyed `fetch(key)`；
- keyed `find(key)` for optional point lookup；
- `containsKey(key)`；
- generated grouped index access, for example `scanByOperation(operationKey)`；
- generated machine frontier index access, for example `scanByMachine(machineId)`；
- application-owned machine availability minimum heap；
- generated optional presence predicate；
- Candidate Scan `findFirst()` / `firstOrThrow()`；
- Candidate Scan `sorted(comparator)` dynamic sort；
- `mutate(key).field(...).commit()`；
- Candidate Scan `update(updater)` for indicator refresh；
- Candidate Scan `remove()` for selected operation candidate cleanup；
...
- `MachineCandidate` 不声明 dispatch order，FCFS、SPT 和 setup 相关 indicator 由 solver loop 在 dispatch 前更新，再通过 Candidate Scan `sorted(comparator)` 动态排序；
...
- Candidate Scan lazy/one-shot terminal evidence；
- ColumnView evidence；

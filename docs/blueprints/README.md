...
- 产品蓝图给出共同心智模型；场景蓝图必须说明 data role、默认推荐路径、条件式替代方案和 application responsibility；
- 未标为算法伪代码的片段按目标 Java 8 使用代码审查，但 Blueprint 不拥有精确 signature；
- reference path 可以为了可读性 materialize，hot path 必须披露 IndexSnapshot、ColumnView、lookup、sort、scratch 与 allocation 边界；
...
场景蓝图不是 SOMA 对业务算法的所有权声明。调度策略、路径搜索、数值模型、游戏规则以及跨表提交顺序与失败处置仍由 application 拥有。

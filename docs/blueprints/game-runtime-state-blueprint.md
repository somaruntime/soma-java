...
| `MapTileDefinitionRow` | dense long-lived input layout | immutable terrain/cost facts | explicit grid-position sort、packed scan |
| `TileOccupancyRow` | dense derived cache | 从 unit position 重建 | paired grid scan、coordinate adapter |
...
`GridPosition` 是 value field，flatten 到 tile row columns。Dense grid 的主要目标访问方式是 grid scan / ordered traversal；它本身不承诺 coordinate O(1) fetch。
...
- game loop 拥有 coordinate adapter、occupancy recovery、damage semantics 和跨 table failure 处置。

### 10.2 采用前证明义务

- 验证 selected-unit `replaceAll + dynamic sort` 的规模边界，并与条件式 keyed frontier 做同语义比较；
- 为 coordinate lookup 选择 dense scan、external grid adapter 或 keyed tile access，并验证其 invariant 和成本；

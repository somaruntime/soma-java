...
类型：Product Projection / User Guide
...
输入事实源：[正式文档入口](../README.md)、当前 `pom.xml`、external Maven fixtures 与 Gate reports
...
`1.0.0`当前仍是private-source release-shaped坐标，尚未自动形成tag、GitHub
Release或Maven发布。consumer只能使用正式提供的release tag；tag尚不可用时，
必须使用Owner提供的immutable commit SHA，或与该SHA绑定且checksum/provenance
可核对的curated source archive。不要把移动中的`develop`、普通repository
snapshot、本机install或历史qualification当作试用基线，也不得将其描述为public
RC、production-ready或Maven Central artifact。
...
- 已授权的完整private repository ref，或同SHA qualification生成并完成
  extracted-archive Maven verify的curated source archive；

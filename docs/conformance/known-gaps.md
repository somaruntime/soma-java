...
|---|---|---|---|---|
| `CF-005` | Evidence | 当前八份性能 baseline 只覆盖指定 Zulu JDK 8/macOS/aarch64 环境与固定 lanes/workload | 其他环境只能得到 `not-applicable`，不能外推为支持矩阵或普遍性能优势 | 保留环境限定和 `claimAllowed=false`；新增环境需独立校准，public claim 需另行授权 |
...
| industrial DataFlow application trace | 已关闭 | `AssignmentSummaryFlow` 只从 authoritative assignments 推导 detached summary；primitive frontier、Problem/Result/API/Schema 与三个性能阈值不变 |
...
| performance baseline 可持续性 | 已关闭 | component=2、reference application=6、public claim=0；baseline 只读、环境感知、strict shape，统一 comparator 不拥有应用 workload |

...
|---|---|---|---|---|---|---|---|
| G01 | 用户能用一句稳定叙事理解 SOMA，并沿一条 canonical journey 完成首次成功 | Product Blueprint；设计宪法；系统架构 | 现有 Blueprint、三个 Example、TV0–TV8 | guides、generated facade、external consumer、docs Gate | 三个 Example | pending | P4/P6 冻结产品叙事与 promotion manifest，P11 原子固化 |
...
| G03 | Runtime 能力可局部替换但语义稳定，hot path 保持 compiler-specialized | 系统架构；Capability/模块边界 Design | TV0–TV8 rejected/accepted 清单 | processor/runtime-core/dataflow；API/codegen/component tests | 三个 Example按需 | pending | 封闭 Capability Set、binding、internal strategy 与 public contract |
...
| G09 | 默认结果完整原子且易推理；大结果可在受限只读场景按需消费 | Result Delivery/Materialization Design | TV3 eager；TV9 待执行 | dataflow/generated contract；correctness/allocation/failure/compatibility | P9 按产品价值决定 | in-progress | preregister、执行并裁决 Eager vs callback-scoped streaming |
...
| G15 | Small/Medium、1M/10M、单表/双表 100M 与 String 的 claim 均有 production-shape evidence | performance/benchmark/validation governance | TV7/TV8；TV9 待执行 | benchmarks/scripts/reports；Zulu 8 local qualification | N/A | pending | 生产实现后建立 bounded profiles；不外推 SLA/support matrix |
...
| P1 | in-progress | 本矩阵建立；TV9 experiment brief、workload identity 和退出条件待 Lab checkpoint | G01–G17 全部保留；唯一新增独立验证仍为 TV9 |
| P2–P11 | pending | 按 Goal 执行 | 不得以耗时、token、代码规模或当前实现反向降低目标 |

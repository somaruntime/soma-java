...
小规模或全局重算场景应保留当前正式示例的 dense workspace。也就是说，`InsertionCandidateRow` 仍然可以是 dense table，并且每轮通过 `replaceAll(batch)` 发布当前 candidate workspace；`by_best_delta` 只是本轮显式 comparator，不进入 Schema。

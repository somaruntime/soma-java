...
| normalized schema/hash | processor normalized model/output | fixture `expected/*.schema.json`、`.sha256` 与 Unicode/negative diagnostics |
...
| benchmark artifact schema | benchmark model/validator | smoke runner、strict validator、negative artifact cases |
...
Implementation Map 的“实现核对基线”指最后一次影响代码、配置、测试或 executable contract 的 commit，不要求等于最新 docs-only commit。若其后只有文档变化，应在审计材料中证明相关实现树未变化；一旦 implementation surface 变化，所有受影响地图必须更新到新的 immutable implementation baseline。

...
| `IndexSnapshot` | detached current-Index sequence + source/epoch identity | stable row identity、live view |
...
`IndexBuffer` 是 internal operation scratch；`IndexSnapshot` 是 public detached sequence。两者都不拥有业务 identity。

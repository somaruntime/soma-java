...
以上差距都已有明确处置；它们尚未实施或受外部事实限制，但不存在借Conformance自动扩权的未裁决项。它们不阻塞文档体系切换，也不会因文档切换而自动关闭。
...
| `CF-007` 文档候选完整性 | 已完成切换前审计 | 32份旧Owner必须按迁移审计原子处置；在正式切换前仍保持candidate/not-enabled |

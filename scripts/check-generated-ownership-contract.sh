...
if grep -E 'ObjectColumn<.*Table|List<.*>Column|Map<.*>Column' \
  ...
  printf '%s\n' 'generated-ownership-contract: collection/facade leaked into live child storage' >&2
  exit 1
fi
if grep -E 'public .+\(int rowIndex([,)])' "$generated_dir"/*Table.java >/dev/null; then
  printf '%s\n' 'generated-ownership-contract: public child facade rowIndex parameter leaked' >&2
  exit 1
fi
if grep -E 'ArrayList|HashMap|Map<' "$registry_source" >/dev/null; then
  printf '%s\n' 'generated-ownership-contract: Java Collection leaked into ownership registry storage' >&2
  exit 1
fi
if ! grep -F 'createOwned(RuntimePlan plan,ChildOwnershipRegistry ownership,TablePlan tablePlan,String path)' \
  ...
  printf '%s\n' 'generated-ownership-contract: owned child did not receive pre-bound TablePlan' >&2
  exit 1
fi
if grep -E 'static .* createOwned\(.*(verifySchemaPlan|toBuilder)' \
    "$generated_dir"/*Table.java >/dev/null; then
  printf '%s\n' 'generated-ownership-contract: owned child repeated schema verification or TablePlan copy' >&2
  exit 1
fi
...
if grep -F 'initialCapacity' "$fixture/target/classes/$schema" >/dev/null; then
  printf '%s\n' 'generated-ownership-contract: runtime capacity hint leaked into logical schema hash' >&2
  exit 1
fi
...
if grep -E 'ChildOwnershipRegistry|OwnedChildTable|Handle' \
  ...
  printf '%s\n' 'generated-ownership-contract: runtime ownership protocol leaked into public facade' >&2
  exit 1
fi
...
if grep -F ' copy();' "$evidence_dir/ParentRowBatch.javap.txt" >/dev/null; then
  printf '%s\n' 'generated-ownership-contract: internal Batch copy leaked into public API' >&2
  exit 1
fi

ownership_javap=$evidence_dir/ownership-generated.javap.txt
types_file=$evidence_dir/ownership-generated-types.txt
...
done <"$types_file" >"$ownership_javap"
cmp "$source_fixture/expected-generated-public.javap.txt" "$ownership_javap"
...
printf '%s\n' "generated-ownership-contract-evidence: $evidence_dir"
printf '%s\n' 'generated-ownership-contract: ok'

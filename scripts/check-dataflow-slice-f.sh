...
"$JAVA_HOME/bin/java" -cp "$classpath" \
  com.hgtech.soma.benchmarks.DataFlowSliceCCheck
"$JAVA_HOME/bin/java" -cp "$classpath" \
  com.hgtech.soma.benchmarks.DataFlowSliceDCheck
"$JAVA_HOME/bin/java" -cp "$classpath" \
  com.hgtech.soma.benchmarks.DataFlowSliceECheck
"$JAVA_HOME/bin/java" -cp "$classpath" \
  com.hgtech.soma.benchmarks.DataFlowSliceFCheck

git diff --check
printf '%s\n' 'dataflow-slice-f-gate: ok'

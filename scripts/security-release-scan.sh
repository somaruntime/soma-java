...
./mvnw -B -ntp -N -Dmaven.repo.local="$repository" \
  org.apache.maven.plugins:maven-help-plugin:3.5.1:effective-pom \
  -Doutput="$evidence_dir/build-plugin-effective-pom.xml" \
  >"$evidence_dir/build-plugin-effective-pom.log"
...

{
  ...
  printf 'buildPluginInventory=effective POM generated by pinned maven-help-plugin 3.5.1\n'
  printf 'knownLimitation=Effective build-plugin configuration is recorded but plugin transitive code is not part of the SOMA runtime artifact\n'

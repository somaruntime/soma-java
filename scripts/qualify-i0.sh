...
if [ -z "${JAVA_HOME:-}" ]; then
    echo "i0-qualification: JAVA_HOME must select the qualified Java 8 JDK" >&2
    exit 1
fi
...
for tool in "$java_cmd" "$javac_cmd" "$jar_cmd" "$javap_cmd"; do
    if [ ! -x "$tool" ]; then
        echo "i0-qualification: missing JDK tool: $tool" >&2
        exit 1
    fi
done
if ! "$java_cmd" -version 2>&1 | grep -q 'version "1\.8\.'; then
    echo "i0-qualification: JAVA_HOME is not Java 8" >&2
    exit 1
fi
if ! "$javac_cmd" -version 2>&1 | grep -q '^javac 1\.8\.'; then
    echo "i0-qualification: JAVA_HOME javac is not Java 8" >&2
    exit 1
fi
...
if [ "$module_count" -ne 2 ] \
    ...
    echo "i0-qualification: reactor must contain exactly the two production modules" >&2
    exit 1
fi
if grep -q '<parent>' "$repo_root/soma-runtime/pom.xml" \
    ...
    echo "i0-qualification: child POMs must be self-contained and root must be non-published" >&2
    exit 1
fi

work_root=$(mktemp -d "${TMPDIR:-/tmp}/soma-i0-qualification.XXXXXX")
cleanup() {
    case "$work_root" in
        */soma-i0-qualification.*) rm -rf -- "$work_root" ;;
        *) echo "i0-qualification: refusing unsafe cleanup target" >&2 ;;
    esac
}
...
if [ -z "$runtime_jar" ] || [ -z "$processor_jar" ]; then
    echo "i0-qualification: production artifacts not found" >&2
    exit 1
fi
...
"$javac_cmd" \
    ...
    -d "$classes" \
    -s "$generated" \
    "$repo_root/tests/i0-consumer/src/main/java/example/i0/schema/package-info.java" \
    "$repo_root/tests/i0-consumer/src/main/java/example/i0/schema/Sample.java" \
    "$repo_root/tests/i0-consumer/src/main/java/example/i0/I0ConsumerMain.java"
...
if "$java_cmd" -classpath "$classes:$mutated_runtime" example.i0.I0ConsumerMain \
    ...
    echo "i0-qualification: same-version runtime artifact mismatch was accepted" >&2
    exit 1
fi
...
cp "$repo_root/tests/i0-regeneration/pom.xml" "$stale_root/pom.xml"
cp -R "$repo_root/tests/i0-regeneration/snapshots/initial/." "$stale_sources"
...
case "$stale_sources" in
    ...
    *) echo "i0-qualification: refusing unsafe stale-source cleanup" >&2; exit 1 ;;
esac
mkdir -p "$stale_sources"
cp -R "$repo_root/tests/i0-regeneration/snapshots/renamed/." "$stale_sources"
mvn -f "$stale_root/pom.xml" clean compile
...
if grep -q '^org/junit/' "$work_root/runtime-jar.txt" \
    ...
    echo "i0-qualification: JUnit leaked into a production artifact" >&2
    exit 1
fi
...
if ! cmp -s "$first_hashes" "$second_hashes"; then
    echo "i0-qualification: clean package artifacts are not reproducible" >&2

...
if grep -E 'class (Inline|Overflow|Evaluation|Plan)' "$scan_source" >/dev/null \
  ...
  printf '%s\n' 'generated-dense-contract: shared plan or direct packed executor regressed' >&2
  exit 1
fi
if grep -E 'new Selection|boolean\[\] seen|operation\+"\.predicate"|Arrays\.copyOf\(kinds,n\+1\)' \
  "$scan_source" >/dev/null; then
  printf '%s\n' 'generated-dense-contract: candidate scan temporary allocation shape regressed' >&2
  exit 1
fi
if grep -E 'java\.util\.stream|Integer\[|new (ArrayList|LinkedList)|for\([^)]*\).*new (Cursor|UpdateCursor)' "$scan_source"; then
  printf '%s\n' 'generated-dense-contract: forbidden candidate scan hot-path source shape' >&2
  exit 1
fi
if grep -E 'java/util/stream|java/lang/(Boolean|Byte|Short|Integer|Long|Float|Double)\.valueOf|java/util/Iterator' "$evidence_dir/ParticleScan.bytecode.txt"; then
  printf '%s\n' 'generated-dense-contract: forbidden candidate scan hot-path bytecode shape' >&2
  exit 1
fi
...
./mvnw -version
printf '%s\n' "generated-dense-contract-evidence: $evidence_dir"
printf '%s\n' 'generated-dense-contract: ok'

...
if [ -z "${JAVA_HOME:-}" ] || [ ! -x "$JAVA_HOME/bin/javac" ]; then
  printf '%s\n' 'value-modifiers-phase5-check: JAVA_HOME must point to a full JDK 8' >&2
  exit 1
fi
...
if [ "$java_specification" != '1.8' ]; then
  printf '%s\n' "value-modifiers-phase5-check: expected Java 8, got $java_specification" >&2
  exit 1
fi
...
for artifact in "$annotations_jar" "$processor_jar"; do
  if [ ! -f "$artifact" ]; then
    printf '%s\n' \
      "value-modifiers-phase5-check: missing artifact $artifact; run reactor package first" >&2
    exit 1
  fi
done

fixture_root=soma-testkit/src/test/fixtures/compiler/value-modifiers-phase5
mkdir -p target
evidence_dir=$(mktemp -d "$root_dir/target/value-modifiers-phase5.XXXXXX")
...
assert_rejected() {
  ...
  if compile_fixture "$fixture" "$output" "$log"; then
    printf '%s\n' \
      "value-modifiers-phase5-check: $fixture fixture unexpectedly compiled" >&2
    exit 1
  fi
...
for diagnostic_log in "$evidence_dir"/*.log; do
  if grep -E 'Exception in thread|^[[:space:]]+at (com\.hgtech|com\.sun\.tools)' \
    ...
    printf '%s\n' \
      "value-modifiers-phase5-check: diagnostic leaked internal stack: $diagnostic_log" >&2
    exit 1
  fi
done
...
printf '%s\n' "value-modifiers-phase5-evidence: $evidence_dir"
printf '%s\n' 'value-modifiers-phase5-check: ok'

...
if [ -z "${JAVA_HOME:-}" ]; then
    echo "i6-qualification: JAVA_HOME must select the qualified Java 8 JDK" >&2
    exit 1
fi
...
for tool in "$java_cmd" "$javac_cmd" "$javap_cmd" "$jar_cmd"; do
    test -x "$tool" || {
        echo "i6-qualification: missing JDK tool: $tool" >&2
        exit 1
    }
done
...
if [ "${SOMA_I6_REUSE_BUILD:-0}" != 1 ]; then
    mvn clean package
fi
...
test -n "$runtime_jar" && test -n "$processor_jar" || {
    echo "i6-qualification: production artifacts not found" >&2
    exit 1
}

work_root=$(mktemp -d "${TMPDIR:-/tmp}/soma-i6-qualification.XXXXXX")
cleanup() {
    case "$work_root" in
        */soma-i6-qualification.*) rm -rf -- "$work_root" ;;
        *) echo "i6-qualification: refusing unsafe cleanup target" >&2 ;;
    esac
}
...
if grep -q 'io.github.somaruntime.soma.internal' \
    ...
    echo "i6-qualification: generated public signature leaks internal type" >&2
    exit 1
fi

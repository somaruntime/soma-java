...
soma_require_supported_jdk rtd-rule-engine-check
...
if grep -R -a -F 'Unresolved compilation problem' \
  ...
  printf '%s\n' \
    'rtd-rule-engine-check: compiler-error stub in external build' >&2
  exit 1
fi
...
if [ "${SOMA_APPLICATION_PERFORMANCE_MODE:-compare}" = calibration ]; then
  ...
  if [ ! -s "$comparator_class" ]; then
    printf '%s\n' \
      "rtd-rule-engine-check: comparator class missing: $comparator_class" >&2
    exit 1
  fi
  "$JAVA_HOME/bin/java" \

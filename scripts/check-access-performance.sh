...
if [ -z "${JAVA_HOME:-}" ] || [ ! -x "$JAVA_HOME/bin/java" ] \
  ...
  printf '%s\n' 'access-component-check: JAVA_HOME must point to a full JDK 8' >&2
  exit 1
fi
...
if [ "$java_specification" != '1.8' ]; then
  printf '%s\n' "access-component-check: expected Java 8, got $java_specification" >&2
  exit 1
fi

mkdir -p target
evidence_dir=$(mktemp -d "$root_dir/target/access-components.XXXXXX")
...
if [ "$record_count" -ne $((40 * forks)) ]; then
  printf '%s\n' \
    "access-component-check: expected $((40 * forks)) records, got $record_count" >&2
  exit 1
fi
for artifact in "$@"; do
  if grep -v -F '"schemaVersion":"soma-access-component-v1"' \
      "$artifact" >/dev/null \
      || grep -v -F '"claimAllowed":false' "$artifact" >/dev/null; then
    printf '%s\n' \
      'access-component-check: invalid schema or claim boundary' >&2
    exit 1
  fi
done
for lane in \
  ...
  if ! grep -F "\"lane\":\"$lane\"" "$@" >/dev/null; then
    printf '%s\n' "access-component-check: missing lane $lane" >&2
    exit 1
  fi
done
...
for class_name in \
  AccessComponentBenchmark \
  AccessComponentArtifactValidator \
...
printf '%s\n' "access-component-baseline: $baseline_status"
printf '%s\n' "access-component-evidence: $evidence_dir"
printf '%s\n' 'access-component-check: ok'

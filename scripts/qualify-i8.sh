...
reject_match() {
    ...
    set -e
    case "$status" in
        0) echo "i8-qualification: forbidden surface detected" >&2; exit 1 ;;
        1) return 0 ;;
        *) exit "$status" ;;
    esac
}
...
if [ -z "${JAVA_HOME:-}" ]; then
    echo "i8-qualification: JAVA_HOME must select the qualified Java 8 JDK" >&2
    exit 1
fi
...
for tool in "$java_cmd" "$javac_cmd" "$jar_cmd"; do
    test -x "$tool" || {
        echo "i8-qualification: missing JDK tool: $tool" >&2
        exit 1
    }
done
...
if [ "${SOMA_I8_REUSE_CORE_BUILD:-0}" != 1 ]; then
    mvn clean package
    SOMA_I7_REUSE_BUILD=1 scripts/qualify-i7.sh
fi
...
if [ "${SOMA_I8_SKIP_BENCHMARK:-0}" != 1 ]; then
    SOMA_I8_REUSE_BUILD=1 scripts/benchmark-i8.sh
fi
SOMA_I8_REUSE_BUILD=1 scripts/package-i8.sh

package_root="$repo_root/target/i8-package"
...
work_root=$(mktemp -d "${TMPDIR:-/tmp}/soma-i8-qualification.XXXXXX")
cleanup() {
    case "$work_root" in */soma-i8-qualification.*) rm -rf -- "$work_root" ;; *) exit 1 ;; esac
}
...
test -s "$archive"
if tar -tzf "$archive" | grep -E '(^|/)(project|tests|scripts|target|\.git)(/|$)|/src/test/'; then
    echo "i8-qualification: source delivery contains internal evidence" >&2
    exit 1
fi

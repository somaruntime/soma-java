...
final class DenseTableSourceGenerator {
    ...
    private String rowsSource(TableSpec table) {
        ...
        out.append("import com.hgtech.soma.runtime.RemoveResult;\n")
                ...
                .append("  public long count(){start(\"rows.count\");long scanned=0L,reached=0L;try{if(!hasSort()){Cursor c=new Cursor(table);int initial=source==null?table.size():source.size();long[] seen=new long[kinds.length];for(int position=0;position<initial&&!limitReached(seen,0,kinds.length);position++){int rowIndex=source==null?position:source.rowAt(position);scanned++;if(matches(rowIndex,c,seen,0,kinds.length))reached++;}table.endSuccess(\"rows.count\",scanned,reached,0L);return reached;}Selection s=select(terminalMaximum());table.endSuccess(\"rows.count\",s.scanned,s.length,0L);return s.length;}catch(SomaRuntimeException f){table.endFailure(\"rows.count\",scanned==0L?attemptedScanned:scanned,reached==0L?attemptedMatched:reached,f.code());throw f;}catch(RuntimeException f){table.abort(\"rows.count\");throw f;}catch(Error f){table.abort(\"rows.count\");throw f;}}\n")
                ...
                .append("  private boolean matchTerminal(Predicate value,boolean any){if(value==null)throw new NullPointerException(\"predicate\");String op=any?\"rows.anyMatch\":\"rows.noneMatch\";start(op);long reached=0L,scanned=0L;try{Cursor c=new Cursor(table);if(!hasSort()){int initial=source==null?table.size():source.size();long[] seen=new long[kinds.length];for(int position=0;position<initial&&!limitReached(seen,0,kinds.length);position++){int rowIndex=source==null?position:source.rowAt(position);scanned++;if(matches(rowIndex,c,seen,0,kinds.length)){reached++;if(test(value,c,rowIndex,op)){table.endSuccess(op,scanned,reached,0L);return true;}}}table.endSuccess(op,scanned,reached,0L);return false;}Selection s=select(terminalMaximum());scanned=s.scanned;for(int i=0;i<s.length;i++){reached++;if(test(value,c,s.rows[i],op)){table.endSuccess(op,scanned,reached,0L);return true;}}table.endSuccess(op,scanned,reached,0L);return false;}catch(SomaRuntimeException f){table.endFailure(op,scanned==0L?attemptedScanned:scanned,reached==0L?attemptedMatched:reached,f.code());throw f;}catch(RuntimeException f){table.abort(op);throw f;}catch(Error f){table.abort(op);throw f;}}\n")
                .append("  public void forEach(Consumer value){if(value==null)throw new NullPointerException(\"consumer\");start(\"rows.forEach\");long reached=0L,scanned=0L;Selection s=null;try{Cursor c=new Cursor(table);if(!hasSort()){int initial=source==null?table.size():source.size();long[] seen=new long[kinds.length];for(int position=0;position<initial&&!limitReached(seen,0,kinds.length);position++){int rowIndex=source==null?position:source.rowAt(position);scanned++;if(!matches(rowIndex,c,seen,0,kinds.length))continue;reached++;c.open(rowIndex);table.beginCallback(\"rows.forEach.consumer\");try{value.accept(c);}catch(SomaRuntimeException failure){throw failure;}catch(RuntimeException callback){throw RuntimeFailures.callbackFailed(").append(q(table.logicalName)).append(",\"rows.forEach\",\"consumer\",callback);}finally{table.endCallback(\"rows.forEach.consumer\");c.close();}}table.endSuccess(\"rows.forEach\",scanned,reached,0L);return;}s=select(terminalMaximum());scanned=s.scanned;for(int i=0;i<s.length;i++){reached++;c.open(s.rows[i]);table.beginCallback(\"rows.forEach.consumer\");try{value.accept(c);}catch(SomaRuntimeException failure){throw failure;}catch(RuntimeException callback){throw RuntimeFailures.callbackFailed(").append(q(table.logicalName)).append(",\"rows.forEach\",\"consumer\",callback);}finally{table.endCallback(\"rows.forEach.consumer\");c.close();}}table.endSuccess(\"rows.forEach\",scanned,reached,0L);}catch(SomaRuntimeException f){table.endFailure(\"rows.forEach\",scanned==0L?attemptedScanned:scanned,reached==0L?attemptedMatched:reached,f.code());throw f;}catch(RuntimeException f){table.abort(\"rows.forEach\");throw f;}catch(Error f){table.abort(\"rows.forEach\");throw f;}}\n")
                ...
                .append("  public List<").append(table.carrierType).append("> fetchAll(MaterializationBudget budget){if(budget==null)throw new NullPointerException(\"budget\");start(\"rows.fetchAll\");try{Selection s=select(terminalMaximum());List<").append(table.carrierType).append("> result=table.materializeRows(s.rows,s.length,budget,\"rows.fetchAll\",true);table.endSuccess(\"rows.fetchAll\",s.scanned,s.length,0L);return result;}catch(SomaRuntimeException f){table.endFailure(\"rows.fetchAll\",attemptedScanned,attemptedMatched,f.code());throw f;}catch(RuntimeException f){table.abort(\"rows.fetchAll\");throw f;}catch(Error f){table.abort(\"rows.fetchAll\");throw f;}}\n")
                .append("  public int[] rowIndexes(){start(\"rows.rowIndexes\");try{Selection s=select(terminalMaximum());int[] result=Arrays.copyOf(s.rows,s.length);table.endSuccess(\"rows.rowIndexes\",s.scanned,s.length,0L);return result;}catch(SomaRuntimeException f){table.endFailure(\"rows.rowIndexes\",attemptedScanned,attemptedMatched,f.code());throw f;}catch(RuntimeException f){table.abort(\"rows.rowIndexes\");throw f;}catch(Error f){table.abort(\"rows.rowIndexes\");throw f;}}\n")
                .append("  public UpdateResult update(Updater value){if(value==null)throw new NullPointerException(\"updater\");table.preflightMutation(\"rows.update\");start(\"rows.update\");long reached=0L;Selection s=null;try{s=select(terminalMaximum());table.prepareUpdateScratch(s.length);table.loadUpdateScratch(s.rows,s.length);MutableCursor c=new MutableCursor(table);for(int i=0;i<s.length;i++){reached++;c.open(i,s.rows[i]);table.beginCallback(\"rows.update.updater\");try{value.update(c);}catch(SomaRuntimeException failure){throw failure;}catch(RuntimeException callback){throw RuntimeFailures.callbackFailed(").append(q(table.logicalName)).append(",\"rows.update\",\"updater\",callback);}finally{table.endCallback(\"rows.update.updater\");c.close();}}long changed=table.publishUpdate(s.rows,s.length);table.endSuccess(\"rows.update\",s.scanned,s.length,changed);return table.updateResult(s.scanned,s.length,changed,0L,rebuiltSidecars());}catch(SomaRuntimeException f){table.endFailure(\"rows.update\",s==null?attemptedScanned:s.scanned,reached==0L&&s==null?attemptedMatched:reached,f.code());throw f;}catch(RuntimeException f){table.abort(\"rows.update\");throw f;}catch(Error f){table.abort(\"rows.update\");throw f;}finally{if(s!=null)table.clearUpdateScratch(s.length);}}\n\n")
                .append("  public RemoveResult remove(){table.preflightMutation(\"rows.remove\");start(\"rows.remove\");Selection s=null;try{s=select(terminalMaximum());RemoveResult result=table.removeSelected(s.rows,s.length,s.scanned,0L,rebuiltSidecars(),\"rows.remove\");table.endSuccess(\"rows.remove\",s.scanned,s.length,s.length);return result;}catch(SomaRuntimeException f){table.endFailure(\"rows.remove\",s==null?attemptedScanned:s.scanned,s==null?attemptedMatched:s.length,f.code());throw f;}catch(RuntimeException f){table.abort(\"rows.remove\");throw f;}catch(Error f){table.abort(\"rows.remove\");throw f;}}\n\n")
                ...
                .append("  private int terminalMaximum(){int first=firstSort(),maximum=Integer.MAX_VALUE;if(first==kinds.length){for(int i=0;i<kinds.length;i++)if(kinds[i]==LIMIT)maximum=(int)Math.min((long)maximum,counts[i]);return maximum;}for(int i=first+1;i<kinds.length;i++){if(kinds[i]!=LIMIT)return Integer.MAX_VALUE;maximum=(int)Math.min((long)maximum,counts[i]);}return maximum;}\n")
